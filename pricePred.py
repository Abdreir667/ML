...
def StochasticGD(X, y, learningRate,perioada=20):
    m = len(y)
    for per in range(perioada):
        inputs=np.array([i for i in range(m)])
        np.random.shuffle(inputs)
        X=X[inputs]
        y=y[inputs]
        for i in range(m):
            for j in range(len(theta[0])):
                theta_sum=(h(X[i],theta)-y[i])*X[i][j]
                theta[0][j]-=learningRate/m*theta_sum
    return theta

theta=StochasticGD(X_train,y_train,learningRate)
print(theta)
